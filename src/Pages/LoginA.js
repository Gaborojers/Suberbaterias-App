...
function LoginA() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:3001/administradores/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        navigate('/menu'); // Cambia '/dashboard' por la ruta que desees
      } else {
      ...
      <div className='login'>
        ...
        <div className='formulario'>
          ...
          <p className="admin-login-link">
            ¿Eres un vendedor? <Link to="/">Inicia sesión aquí</Link>
          </p>
        </div>
      </div>
    </div>
